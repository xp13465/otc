...
//触发一个本对象方法，模拟事件触发
    ...
    createViewByModule:function(module){
        var me = this;
        ...
        var viewpanel = Ext.create('ui.view.'+ module + '.'+module+'View', { ctrl : this});
    ...
    $askYestNoPrompt : function(title,msg,fn){
        // var theparams = {
        //     title: '询问',
        //     msg: '确定删除吗？',
        //     fn: function(btn){
    ...
    $chuliSearchParams : function(params){
        var me = this;
        var _listparam = me.$getConfig('_listParams');
        if(_listparam){
            Ext.apply(params , _listparam);
        }
        return params;
    ...
    $defaultSearchFn : function(btn,me){
        ...
        //Ext.apply(data , {page : 1 , start : 0});
        store.currentPage = 1;
        store.load({params : data});  //待实现传参load
    ...
    $fireGridLoadList : function(){
        for(var i in this.fieldsArray){
            if(this.fieldsArray[i].submitBtn){alert('ddd');
