...
Ext.define('ui.view.investmentproductaudit.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.investmentproductauditpagecentergrid',
    requires :[
        'ui.extend.base.Grid',
    ...
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.investmentproductaudit.list').url;
        this.store = Ext.create( 'ui.store.investmentproductStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.investmentproductaudit.record.json",
            })
        });
//定义checkboxModel
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.investmentproductauditlistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
