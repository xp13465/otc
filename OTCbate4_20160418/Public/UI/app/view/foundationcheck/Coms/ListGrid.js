...
Ext.define('ui.view.foundationcheck.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.foundationcheckpagecentergrid',
    requires :[
        'ui.extend.base.Grid',
    ...
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.foundationcheck.list').url;
        this.store = Ext.create( 'ui.store.productStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.foundationcheck.record.json",
            })
        });
//定义checkboxModel
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.foundationchecklistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
