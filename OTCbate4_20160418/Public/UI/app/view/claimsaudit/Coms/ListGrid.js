...
Ext.define('ui.view.claimsaudit.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.claimsauditpagecentergrid',
    requires :[
        'ui.extend.base.Grid',
    ...
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.claimsaudit.list').url;
        this.store = Ext.create( 'ui.store.debtStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.claimsaudit.record.json",
            })
        });
//定义checkboxModel
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.claimsauditlistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
