...
Ext.define('ui.ctrl.sysCtrl', {
        ...
        this.control({
            ...
            "ndeskcenter" : {
                render : function(){
                    ...
                    $("#main-message-box .panel-hide-show-btn .hs-btn-a").click(function(e){
                        var screenHeight = $(window).height();
                        var screenWidth = $(window).width();
    ...
    nodeitemclick : function(me, record, item, index, e, Opts ){
        ...
        if(Ext.isEmpty(record.data.module)){
            ...
            if(record.data.url){
				var windowOpen =  window.open(record.data.url);
				if (typeof( windowOpen) == 'undefined' || typeof( window.open) == 'undefined') {
					alert('窗口无法打开，请检查你的浏览器设置。\n\n Safari浏设置:关闭阻止弹出式窗口。')
					// $("#startmenudiv .clearfix dt").find("a[attr='"+record.data.node_id+"']").trigger("click")
				} 
                return;
            }
