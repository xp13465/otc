...
Ext.define('ui.ctrl.productinvestmentrecord.productinvestmentrecordCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.productinvestmentrecord.productinvestmentrecordConf',
        'ui.view.productinvestmentrecord.productinvestmentrecordView',
        'ui.view.productinvestmentrecord.Coms.viewPanel'
    ],
    views : [
        'ui.view.productinvestmentrecord.productinvestmentrecordView'
    ],
    ...
    refs:[
        {ref:'productinvestmentrecordWin' , selector:'productinvestmentrecordwindow'}
    ],

    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.productinvestmentrecord.productinvestmentrecordConf');
        this.callParent(arguments);
    },

    init:function(){
        this.control({      //这里的this相当于这个控制层
            'productinvestmentrecordpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新建角色+' , pmsCode : 'productinvestmentrecord.add' }
    //‘.’替换为 $  ,方法前加__
    __productinvestmentrecord$add : function( btna , params ){
        var me = this;
    ...
    __productinvestmentrecord$cexiao : function( btna , params ){
        var me = this;
        ...
        if(validateFlag){
            me.$askYestNo({
                ...
                yes : function(){
                    me.$requestAjax({
                        url     :   me.$getUrlstrByUrlcode('set.productinvestmentrecord.delete'),
                        method :    'POST',
                        params :    postParams,
                        scope  :    me,
                        backParams: {},
    ...
    __productinvestmentrecord$publish : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        // if(params.status == 1){
        //     Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.productinvestmentrecord.publish'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __productinvestmentrecord$view : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __productinvestmentrecord$edit : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __productinvestmentrecord$approve : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 2){
            Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.productinvestmentrecord.change'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __productinvestmentrecord$reject : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 3){
            Ext.Msg.show({
    ...
    */
    __editPanel$save : function( btn , infopanel ){
        ...
        me.$requestFromDataAjax(
            'get.productinvestmentrecord.edit',
            editparams,
            infopanel,
            function(params){
    ...
    __view_main_render : function(thepanel){
        ...
        me.$requestFromDataAjax(
            'get.productinvestmentrecord.view',
            mydata,
            thepanel
        );
    },
