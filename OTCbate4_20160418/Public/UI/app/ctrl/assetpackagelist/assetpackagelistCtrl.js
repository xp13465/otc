...
Ext.define('ui.ctrl.assetpackagelist.assetpackagelistCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.assetpackagelist.assetpackagelistConf',
        'ui.view.assetpackagelist.assetpackagelistView',
        'ui.view.assetpackagelist.Coms.viewPanel'
    ],
    views : [
        'ui.view.assetpackagelist.assetpackagelistView'
    ],
    ...
    refs:[
        {ref:'assetpackagelistWin' , selector:'assetpackagelistwindow'}
    ],

    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.assetpackagelist.assetpackagelistConf');
        this.callParent(arguments);
    },

    init:function(){
        this.control({      //这里的this相当于这个控制层
            'assetpackagelistpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新建角色+' , pmsCode : 'assetpackagelist.add' }
    //‘.’替换为 $  ,方法前加__
    __assetpackagelist$add : function( btna , params ){
        var me = this;
    ...
    __assetpackagelist$cexiao : function( btna , params ){
        var me = this;
        ...
        if(validateFlag){
            me.$askYestNo({
                ...
                yes : function(){
                    me.$requestAjax({
                        url     :   me.$getUrlstrByUrlcode('set.assetpackagelist.delete'),
                        method :    'POST',
                        params :    postParams,
                        scope  :    me,
                        backParams: {},
    ...
    __assetpackagelist$publish : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        // if(params.status == 1){
        //     Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.assetpackagelist.publish'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __assetpackagelist$view : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
         ...
         me.$showOptionsPanel('view', param ,function( panel ,win ){
            ...
            Ext.Loader.require([
                ...
                viewPanel.on('render',function(){
                    ...
                    store.load({id : param.id});
                    //modelX.$fireGridLoadList();
                });
                panel.formsList[0].add(viewPanel);
                panel.doLayout();
    ...
    __assetpackagelist$edit : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __assetpackagelist$approve : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 2){
            Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.assetpackagelist.change'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __assetpackagelist$reject : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 3){
            Ext.Msg.show({
