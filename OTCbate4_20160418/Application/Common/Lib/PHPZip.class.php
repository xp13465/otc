...
namespace Common\Lib;
    class PHPZip
    {
        ...
        // $archive->ZipAndDownload("需压缩的文件所在目录");
        // ------------------------------------------------------ //
        public function ZipAndDownloadFiles($filelist)
        {
            if(@!function_exists('gzcompress')){ return; }
            ...
            {
				$code = mb_detect_encoding($filename,array('ASSCII','GB2312','UTF-8'));
				if($code =="UTF-8"){
            ...
            $out = $this->file();
			$out_filename = "POS扫描件".date("YmdHis", time()).'.zip';
            ...
            @header('Content-Type: application/zip');
            // @header('Content-Disposition: attachment ; filename=POS扫描件'.date("YmdHis", time()).'.zip');
